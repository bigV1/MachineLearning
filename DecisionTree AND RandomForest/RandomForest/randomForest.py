...
def tes_split(index, value, dataset):
...
def get_split(dataset, n_features):
    ...
    for index in features:                    # 在 n_features 个特征中选出最优的特征索引，并没有遍历所有特征，从而保证了每课决策树的差异性
        for row in dataset:
            groups = tes_split(index, row[index], dataset)  # groups=(left, right), row[index] 遍历每一行 index 索引下的特征值作为分类值 value, 找出最优的分类特征和特征值
...
if __name__ == '__main__':
    ...
    # 加载数据
    dataset = loadDataSet('input/classify/sonar-all-data.txt')
    # print dataset
