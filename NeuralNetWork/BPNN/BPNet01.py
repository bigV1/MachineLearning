...
def bpNet(dataSet, classLabels):
    ...
    hi_wb = mat(mergMatrix(mat(hi_w), mat(hi_b)))

    # 输出层参数
    out_w = 2.0 * (random.rand(nOut, nHidden) - 0.5)
    out_b = 2.0 * (random.rand(nOut, 1) - 0.5)
    out_wb = mat(mergMatrix(mat(out_w), mat(out_b)))
    # 默认旧权值
    dout_wbOld = 0.0;
    dhi_wbOld = 0.0
    ...
    for i in range(maxiter):
        ...
        hi2out = mergMatrix(hi_output.T, ones((nSampNum, 1))).T
...
def runTest():
    # 数据集
    dataSet, classLabels = loadDataSet("testSet2.txt")  # 初始化时第1列为全1向量, studentTest.txt
    dataSet = normalize(mat(dataSet))
    ...
    # BP神经网络进行数据分类
    errRec, WEX, wex = bpNet(dataSet, classLabels)

    # 计算和绘制分类线
    x, z = BPClassfier(-3.0, 3.0, WEX, wex)
